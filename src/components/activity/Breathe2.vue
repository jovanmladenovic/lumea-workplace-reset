<script setup lang="ts">
  ...
  const animateRound = async () => {
    ...
    await animate([lungRef.value], {
      scale: 1.1,
      duration: BASE_ANIMATION_DURATION * 0.4,
      easing: 'easeInOutQuad',
    });

    await sleep(BASE_ANIMATION_DURATION * 0.2);
    ...
    await sleep(BASE_ANIMATION_DURATION * 0.4);
...
<template>
  ...
  >
    <div ref="inhaleRef" :class="textClasses">Two short inhales</div>
    ...
    <div ref="exhaleRef" :class="textClasses">Long exhale</div>
  </Animation>
</template>
