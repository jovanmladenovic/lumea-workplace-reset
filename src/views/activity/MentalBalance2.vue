<script setup lang="ts">
  ...
  import MentalBalance2Step2 from '@/components/activity/MentalBalance2Step2.vue';
  import MentalBalance2Step3 from '@/components/activity/MentalBalance2Step3.vue';
  import MentalBalance2Step4 from '@/components/activity/MentalBalance2Step4.vue';
  import MentalBalance2Step5 from '@/components/activity/MentalBalance2Step5.vue';

  const blobsBackgroundRef = ref<InstanceType<typeof BlobsBackground>>();

  const step1Ref = ref<InstanceType<typeof ActivityIntro>>();
  const step2Ref = ref<InstanceType<typeof MentalBalance2Step2>>();
  const step3Ref = ref<InstanceType<typeof MentalBalance2Step3>>();
  const step4Ref = ref<InstanceType<typeof MentalBalance2Step4>>();
  const step5Ref = ref<InstanceType<typeof MentalBalance2Step5>>();
  ...
  const introMessages: { text: string; class?: string }[] = [
    {
      text: `Stress isn’t failure. It’s <b>overload</b>.`,
      class: 'text-xl px-[140px]',
    },
    {
      text: `Let’s <b>help your system</b> catch up with your mind.`,
      class: 'text-xl px-[140px]',
    },
  ];
  ...
  onMounted(async () => {
    ...
    const allSteps = [
      ...
      () => initStep5().promise,
      async () => {
        await blobsBackgroundRef.value?.animateOut();
...
<template>
  <DefaultLayout>
    ...
    <div class="relative z-10 text-black">
      <ActivityIntro v-if="showStep1" ref="step1Ref" :messages="introMessages" title="Grounding" />
      <MentalBalance2Step2 v-if="showStep2" ref="step2Ref" class="absolute top-0 left-0 right-0" />
      <MentalBalance2Step3 v-if="showStep3" ref="step3Ref" class="absolute top-0 left-0 right-0" />
      <MentalBalance2Step4 v-if="showStep4" ref="step4Ref" class="absolute top-0 left-0 right-0" />
      <MentalBalance2Step5 v-if="showStep5" ref="step5Ref" class="absolute top-0 left-0 right-0" />
    </div>
  </DefaultLayout>
</template>
